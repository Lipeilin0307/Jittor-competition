...
Interface (fixed — other code depends on it, do not change signatures):
    ...
    class CFStats:
        def __init__(self, src, dst, node_features):
        ...
        def batch_features(self, src_arr, cand_mat) -> np.ndarray
            ...
            # cand_mat: [N, K] int64 (raw dst ids)
            # returns float32 [N, K, 8], NaN-free; rows whose src has an empty
            # history (unseen src) are all 0.
...
class CFStats:
    ...
    def batch_features(self, src_arr: np.ndarray, cand_mat: np.ndarray) -> np.ndarray:
        ...
        cands = cand_mat[rows]                            # (m, K) raw dst ids
        c_ok = (cands >= 0) & (cands < self.num_nodes)
        cands_clip32 = np.clip(cands, 0, self.num_nodes - 1).astype(np.int32)

        # ---------------- cooc block (dims 0-3) ----------------
        # Expand every (row, cand) pair into L_row (d, c) term queries, laid out
        # as contiguous groups in (row, k) row-major order. All int32/uint32.
        ...
        out[rows, :, 7] = np.log1p(Lf)[:, None].astype(np.float32)
        ...
        out[rows, :, 4] = np.einsum('md,mkd->mk', mean_u, cand_u).astype(np.float32)
        ...
        out[rows, :, 5] = dim5.astype(np.float32)
        out[rows, :, 6] = dim6.astype(np.float32)
        return out
...
if __name__ == '__main__':
    ...
    # ======================================================================
    # 2) Realistic 2048 x 32 benchmark on the full ds3 train graph
    # ======================================================================
    here = os.path.dirname(os.path.abspath(__file__))
    train_csv = os.path.join(here, 'dataset3', 'train.csv')
    n2v_path = os.path.join(here, 'node_features_dataset3_n2v256.npy')
    if os.path.exists(train_csv) and os.path.exists(n2v_path):
        import pandas as pd
        frame = pd.read_csv(train_csv)
        src_all = frame['src'].to_numpy(np.int64)
        dst_all = frame['dst'].to_numpy(np.int64)
        nf = np.load(n2v_path).astype(np.float32)
        big = CFStats(src_all, dst_all, nf)
        print(f'real build: edges={big.num_edges}, srcs={len(big.uniq_src)}, '
              f'cooc_nnz={big.cooc_nnz}, table={"hash" if big.table is not None else "csr"}, '
              f'build={big.build_seconds:.1f}s')
        ...
        def make_batch(seed):
            ...
            qc = np.concatenate([pos[:, None], neg], axis=1)
            return qs, qc
        ...
        def timed(qs, qc, label):
            ...
            assert feats.shape == (2048, 32, 8) and np.isfinite(feats).all()
        ...
        qs2, qc2 = make_batch(2)
        timed(qs2, qc2, 'warm batch  (new batch)')
        qs3, qc3 = make_batch(3)
        timed(qs3, qc3, 'warm batch  (new batch #2)')
    else:
        print('dataset3 files not found next to the module; skipping real-data benchmark')
