...
class GraphMixerModel(nn.Module):
    ...
    def __init__(self, src_count, dst_count, hidden_dim, initial_features,
        ...
        heuristic_dim = 3 if self.use_heuristics else 0
        self.scorer = MLPScorer(hidden_dim, mlp_ratio, dropout, heuristic_dim)
    ...
    def _get_heuristic_features(self, src_np, cand_np):
        ...
        Returns (B, N, 3) tensor with [degree, popularity, edge_count].
        """
        if not self.use_heuristics:
            return None
        B, N = cand_np.shape
        num_nodes = self.heuristic_degree.shape[0]
        h = np.zeros((B, N, 3), dtype=np.float32)
        # clip candidate indices to valid range [0, num_nodes-1]
        cand_clipped = np.clip(cand_np, 0, num_nodes - 1)
        h[:, :, 0] = self.heuristic_degree[cand_clipped]
        h[:, :, 1] = self.heuristic_popularity[cand_clipped]
        # edge_count: dict lookup per pair (unseen nodes get 0)
        for i in range(B):
            for j in range(N):
                h[i, j, 2] = self.heuristic_edge_count.get((int(src_np[i]), int(cand_np[i, j])), 0.0)
        if self.edge_count_max > 1.0:
            h[:, :, 2] = h[:, :, 2] / self.edge_count_max
        return jt.array(h)

    def _score(self, src_np, hist_np, cand_np, time_gap_np=None):
        """
    ...
    def execute(self, src_np, hist_np, cand_np, time_gap_np=None):
        """Inference: return raw scores (B, N)."""
        return self._score(src_np, hist_np, cand_np, time_gap_np)

    forward = execute

    def calculate_loss(self, src_np, hist_np, cand_np, time_gap_np=None):
        """
        Training loss: listwise cross_entropy + BPR.
        cand_np: (B, 1+num_neg) = [pos, neg1, neg2, ...]
        """
        scores = self._score(src_np, hist_np, cand_np, time_gap_np)  # (B, N)
        B, N = scores.shape
