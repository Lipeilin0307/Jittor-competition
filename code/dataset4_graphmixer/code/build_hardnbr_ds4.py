"""
build_hardnbr_ds4.py
====================
Offline top-K cosine neighbor table over the ds4 dst word2vec embeddings
(node_features_ds4_dst.npy, 862248 x 256, rows = dense dst ids). Used by the
training script's cos-hard negative sampler (--hard_neg_ratio).

Method:
  * L2-normalize rows (zero-norm rows stay zero -> zero scores).
  * GPU (jittor CUDA) batched matmul: row batches of --row_batch dst rows vs
    column tiles of --col_tile dsts; fp32 scores (fp16 measured to corrupt
    top-K selection: boundary ties shuffle ranks, overlap drops to ~200/256).
  ...
  * Rows 0 (padding) and 862247 (UNK) and any zero-norm row are emitted as
    all-zero neighbor lists (training falls back to popularity sampling).

Output: hardnbr_ds4.npy (862248, K) int32, K default 256.

Budget target ~10-15 min on RTX3070 Laptop. --selftest runs a tiny exact
check; --spot_check N verifies N random rows against fp32 brute force.
"""
import argparse
import os
...
def _topk_tile(scores, k, nthreads=8):
    """Row-wise top-k of a (R, C) fp16 score block via threaded argpartition.
...
def build(dst_features, out_path, topk=256, row_batch=4096, col_tile=131072,
    ...
    # padding / UNK / zero-norm rows -> all-zero neighbor lists
    out[zero_rows] = 0
    out[0] = 0
    if n > 862_247:
        out[862_247] = 0
    ...
    if spot_check > 0:
        ...
        rows = rng.choice(np.arange(1, min(n_rows, n)), size=spot_check,
                          replace=False)
        worst = topk
        worst_gap = 0.0
        for r in rows:
...
def main():
    ...
    ap.add_argument('--dst_features', default='./node_features_ds4_dst.npy')
    ap.add_argument('--out', default='./hardnbr_ds4.npy')
